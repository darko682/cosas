...
import static rommel.Interfaz.areaConsola;
...
class TokenAsignaciones
{
	...
	public static void InsertarSimbolo(Token identificador, int tipo)
	{
		...
		System.err.println("Insertar simbolo");
                if(tabla.containsKey(identificador.image)){
                      lexico.contador.addElement("variable repetida: "+identificador.image.toString()+"\n");
                }else {tabla.put(identificador.image, tipo);
                }
	 }
	...
	public static String checkAsing(Token TokenIzq, Token TokenAsig)
	{
		...
		int tipoIdent1;
		int tipoIdent2;
                System.out.println(TokenIzq.toString()+"********"+TokenAsig);
		...
		if(TokenIzq.kind != 42 && TokenIzq.kind != 44)		
		{
			try 
			{
			...
			System.out.println("error identificador no declarado");
		...
		else if(TokenAsig.kind == 42 || TokenAsig.kind == 42 || TokenAsig.kind == 46 || TokenAsig.kind == 47)
		...
		else if(tipoIdent1 == 44) //Flotante
		{
	  ...
	  //Tabla que almacenara los tokens declarados
            tabla = new Hashtable();
	  //Listas que guardaran los tipos compatibles de las variables
          intComp = new ArrayList();
	  decComp = new ArrayList();
	  strComp = new ArrayList();
	  chrComp = new ArrayList();         
        }
